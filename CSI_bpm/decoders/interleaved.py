    ...
    def get_pd_csi(self):
        ...
        for sample in range(len(self.csi)):
            for subcarrier in range(len(self.csi[sample])):
                if subcarrier not in nulls[self.bandwidth] and subcarrier not in pilots[self.bandwidth]:
                    if subcarrier in csi:
                        csi[subcarrier] = np.append(csi[subcarrier], self.get_csi(sample, True, True)[subcarrier])
                    else:
                        csi[subcarrier] = np.array(self.get_csi(sample, True, True)[subcarrier])
